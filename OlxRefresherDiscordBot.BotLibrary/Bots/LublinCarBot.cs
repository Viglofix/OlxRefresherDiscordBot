...
public class LublinCarBot : IBotCar
{
    ...
    public LublinCarBot(IDiscordClientService discordClientService, ContractInteractivityConfigurationService interactivityConfigurationService, ICommandsNextConfigurationService commandsNextConfigurationService, IAuthorizationJson authorizationJson)
    {
        _discordClientService = discordClientService;
    }
    public async Task BotRunner(string configFileName)
    {
        var discordClient = await _discordClientService.GetDiscordClient(configFileName);

        string ConfigFileName = "configBotCar.json";
        var path = new CurrentDirectoryManager(ConfigFileName).CurrentPath;
        string jsonContent = await JsonFileReadManager<ConfigJsonBot>.GetJsonContent(path!);
        ...
        while (true)
        {
           ...
           // deserializationOptions.Converters.Add(new CustomDateTimeConverter());

            LublinCarModel? obj = null;
            try
            {
                obj = JsonSerializer.Deserialize<LublinCarModel>(result!);
            }
            ...
            {
                Console.WriteLine("403 forbidden occured");
                continue;
            }
           ...
           .FirstOrDefault()!.url;
            ...
            if(price is null)
            {
                price = obj!.data
                 .Where(x => x.promotion.top_ad == false)
                 .FirstOrDefault()!.Params[1].value.value;
            }
